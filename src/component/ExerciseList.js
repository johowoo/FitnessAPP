...
export class _ExerciseList extends PureComponent {
  ...
  handlePress = async () => {
    if (this.state.cardioMinutes && this.state.cardioMinutes !== "0") {
      const re = /^[0-9\b]+$/;
      const result = re.test(this.state.cardioMinutes);
      if (!result) {
        this.setState({
      ...
      this.props.workoutSetsData.forEach(async item => {
        if (item.exercise === this.state.selectedExercise) {
          ...
          await this.setState({
            ...
            reminderContentDuplicated:
              "This exercise has been already added to today's workout",
            hideConfirmButtonDuplicated: true,
          });
      ...
      <View
          ...
          <View
            ...
            <View style={{ flex: 0.1, marginTop: 12, marginRight: 15 }}>
              <ApslButton
                ...
                children={
                  <Icon name="cancel" size={35} key="cancel" color={"#c69"} />
                }
              />
            </View>
          </View>
        ...
        <LinearGradient colors={["#219dd5", "#51c0bb"]} style={{ flex: 1 }}>
          ...
          <Modal
            ...
            {this.state.showReminderModalDuplicated && (
              <ReminderModal
          ...
          <TouchableHighlight
            style={{ flex: 1 }}
            onPress={this.handleBlur.bind(this)}>
            <View style={{ flex: 1 }} />
          </TouchableHighlight>
...
const mapActionToProps = dispatch => ({
  addExerciseToSectionList: data =>
    dispatch(addExerciseToSectionListAction(data)),
  deleteExerciseFromSectionList: data =>
    dispatch(deleteExerciseFromSectionListAction(data)),
});
