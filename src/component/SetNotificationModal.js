...
import {
  getUnixTimeByDay,
  getTimeDifferenceFromZeroOfToday,
} from "../utils/unixTimes";
...
class _SetNotificationModal extends Component {
  ...
  handleTimePicked = async date => {
    await this.setState({
      timeChosen: `${formatMonthandDay(date.getHours())}:${formatMonthandDay(
        date.getMinutes()
      )}`,
      timeDifferenceFromZeroOfToday: getTimeDifferenceFromZeroOfToday(
        date.getTime()
      ),
    });
    // await this.setState({
      ...
      <View>
        <TouchableOpacity
          onPress={async () => {
            if (!item.isChosen) {
              if (
                !this.state.scheduleTimeArr.includes(getUnixTimeByDay(index))
              ) {
                await this.setState({
                  scheduleTimeArr: [
                    ...this.state.scheduleTimeArr,
                    getUnixTimeByDay(index),
                  ],
                });
              }
            } else {
            ...
            await this.props.chooseDayInWeek({
              day: item.day,
  ...
  handleConfirmNotification = async () => {
    ...
    this.state.scheduleTimeArr.forEach(item => {
      ...
      for (let i = 0; i < this.state.validatePeriodCount; i++) {
        const schedulingOptions = {
          time:
            item + this.state.timeDifferenceFromZeroOfToday + oneDayTime * i,
        };
...
export const SetNotificationModal = connect(
  mapStateToProps,
  mapActionsToProps
)(_SetNotificationModal);
