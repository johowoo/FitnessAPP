...
export const editHistoryExercisesList = (
  ...
  switch (action.type) {
    case types.ADD_EXERCISES_TO_EXERCISE_LIST_OF_WORKOUT_HISTORY:
      ...
      const currentDateExerciseListCopy = state.workoutHistoryExerciseList[
        action.payload.date
      ]
        ? JSON.parse(
            JSON.stringify(
              state.workoutHistoryExerciseList[action.payload.date]
            )
          )
        : [];
    ...
    case types.ADD_WEIGHT_REPS_TO_EXERCISE_IN_CALENDAR_HISTORY:
      const stateCopy = JSON.parse(
        JSON.stringify(state.workoutHistoryExerciseList[action.payload.date])
      );
      stateCopy.map(item => {
    ...
    case types.DELETE_EXERCISES_FROM_EXERCISE_LIST_OF_WORKOUT_HISTORY:
      ...
      const emptyState = [];
      state.workoutHistoryExerciseList[action.payload.date].map(
        (item, index) => {
          if (parseInt(item.time, 10) !== parseInt(action.payload.time, 10)) {
    ...
    case types.EDIT_WEIGHT_REPS_IN_WORKOUT_OF_CALENDAR_HISTORY_ACTION:
      const stateCopyForEditAll = JSON.parse(
        JSON.stringify(state.workoutHistoryExerciseList[action.payload.date])
      );
