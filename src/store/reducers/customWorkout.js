...
import {
  initialExerciseSets,
  initialExerciseCategory,
} from "../../initialExerciseSets";
...
for (const key in initialExerciseSets) {
...
export const customWorkout = (state = defaultState, action) => {
  switch (action.type) {
    case types.ADD_EXERCISE_TO_SECTIONLIST:
      ...
      const customWorkoutSetsCopy = JSON.parse(
        JSON.stringify(state.customWorkoutSets)
      );
    ...
    case types.ADD_EXERCISE_SET_TO_CURRENT_WORKOUT:
      ...
      const addableCopy = JSON.parse(
        JSON.stringify(state.customWorkoutAddable)
      );
      addableCopy[action.payload.category] = false;
      // console.warn("addableCopy[action.payload]", addableCopy);
      return {
        ...state,
        customWorkoutAddable: addableCopy,
      };
    case types.CLEAR_CURRENT_WORKOUT:
      ...
      return {
        ...state,
        customWorkoutAddable: customWorkoutAddableEmpty,
      };

    //todo
    case types.ADD_WEIGHT_REPS_TO_EXERCISE_IN_LIBRARY:
      const customWorkoutSetsForGivenCategoryCopy = JSON.parse(
        JSON.stringify(
          state.customWorkoutSets[action.payload.selectedExerciseCategory]
        )
      );
      ...
      return {
        ...
        customWorkoutSets: {
          ...
          [action.payload
            .selectedExerciseCategory]: customWorkoutSetsForGivenCategoryCopy,
        },
      };
    case types.EDIT_WEIGHT_REPS_IN_WORKOUT_OF_LIBRARY:
      const editWorkoutSetsForGivenCategoryCopy = JSON.parse(
        JSON.stringify(
          state.customWorkoutSets[action.payload.selectedExerciseCategory]
        )
      );
      ...
      return {
        ...
        customWorkoutSets: {
          ...state.customWorkoutSets,
          [action.payload
            .selectedExerciseCategory]: editWorkoutSetsForGivenCategoryCopy,
        },
      };
    case types.DELETE_EXERCISE_FROM_WORKOUT_LIST_OF_LIBRARY:
      const deleteWorkoutSetsForGivenCategoryCopy = JSON.parse(
        JSON.stringify(
          state.customWorkoutSets[action.payload.selectedExerciseCategory]
        )
      );
      ...
      return {
        ...
        customWorkoutSets: {
          ...state.customWorkoutSets,
          [action.payload
            .selectedExerciseCategory]: deleteWorkoutSetsForGivenCategoryCopy,
        },
      };
    case types.ADD_EXERCISE_FROM_EXERCISE_MODAL_TO_CATEGORY_OF_LIBRARY:
      ...
      return {
        ...
        customWorkoutSets: {
          ...state.customWorkoutSets,
          [action.payload.selectedExerciseCategory]: [
            ...state.customWorkoutSets[action.payload.selectedExerciseCategory],
            action.payload,
          ],
        },
    ...
    case types.DELETE_CATEGORY_FROM_EDIT_LIBRARY:
      ...
      for (const i in state.customWorkoutAddable) {
        if (
          state.customWorkoutAddable.hasOwnProperty(i) &&
          i !== action.payload
        ) {
          emptyState.customWorkoutAddable[i] = state.customWorkoutAddable[i];
        }
      }
      for (const i in state.customWorkoutSets) {
    ...
    case types.RESET_CUSTOM_WORKOUT_ADDABLE:
      const customWorkoutAddableCopy = JSON.parse(
        JSON.stringify(state.customWorkoutAddable)
      );
      for (const key in customWorkoutAddableCopy) {
